...
public class DiscjockeyCommand {
                ...
                literal("discjockey")
                        ...
                        .then(literal("reload")
                                .executes(context -> {
                                    if (!isLoading(context)) {
                                        ...
                                        context.getSource().sendMessage(Text.translatable(Main.MOD_ID+".reloading"));
                        ...
                        .then(literal("stop")
                                .executes(context -> {
                                    if (Main.SONG_PLAYER.running) {
                                        ...
                                        context.getSource().sendMessage(Text.translatable(Main.MOD_ID+".stopped_playing", Main.SONG_PLAYER.song));
                                        return 1;
                                    }
                        ...
                        .then(literal("speed")
                                .then(argument("speed", FloatArgumentType.floatArg(0.0001F, 15.0F))
                                        ...
                                        .executes(context -> {
                                            ...
                                            context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".speed_changed", Main.SONG_PLAYER.speed));
                                            return 0;
                                        })
                                )
                        )
                        .then(literal("info")
                                .executes(context -> {
                                    ...
                                    if (!Main.SONG_PLAYER.running) {
                                        context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".info_not_running", Main.SONG_PLAYER.speed));
                                        return 0;
                                    }
                                    if (!Main.SONG_PLAYER.tuned) {
                                        context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".info_tuning", Main.SONG_PLAYER.song.displayName, Main.SONG_PLAYER.speed));
                                        return 0;
                                    }else if(!Main.SONG_PLAYER.didSongReachEnd) {
                                        context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".info_playing", formatTimestamp((int) Main.SONG_PLAYER.getSongElapsedSeconds()), formatTimestamp((int) Main.SONG_PLAYER.song.getLengthInSeconds()), Main.SONG_PLAYER.song.displayName, Main.SONG_PLAYER.speed));
                                        return 0;
                                    }else {
                                        context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".info_finished", Main.SONG_PLAYER.song != null ? Main.SONG_PLAYER.song.displayName : "???", Main.SONG_PLAYER.speed));
                                        return 0;
                                    }
                                })
                        )
                        .then(literal("remapInstruments")
                                .executes(context -> {
                                    context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".instrument_info"));
                                    return 0;
                                })
                                .then(literal("map")
                                                ...
                                                .then(argument("newInstrument", StringArgumentType.word())
                                                        ...
                                                        .executes(context -> {
                                                            ...
                                                            if(originalInstrument == null && !originalInstrumentStr.equalsIgnoreCase("all")) {
                                                                context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".invalid_instrument", originalInstrumentStr));
                                                                return 0;
                                                            }

                                                            if(newInstrument == null && !newInstrumentStr.equalsIgnoreCase("nothing")) {
                                                                context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".invalid_instrument", newInstrumentStr));
                                                                return 0;
                                                            }
                                                            ...
                                                            if(originalInstrument == null) {
                                                                ...
                                                                context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".instrument_mapped_all", newInstrumentStr.toLowerCase()));
                                                            }else {
                                                                Main.SONG_PLAYER.instrumentMap.put(originalInstrument, newInstrument);
                                                                context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".instrument_mapped", originalInstrumentStr.toLowerCase(), newInstrumentStr.toLowerCase()));
                                                            }
                                                            return 1;
                                                        })
                                                )
                                ...
                                .then(literal("unmap")
                                        .then(argument("instrument", StringArgumentType.word())
                                                ...
                                                .executes(context -> {
                                                    ...
                                                    if(instrument == null) {
                                                        context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".invalid_instrument", instrumentStr));
                                                        return 0;
                                                    }

                                                    Main.SONG_PLAYER.instrumentMap.remove(instrument);
                                                    context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".instrument_unmapped", instrumentStr.toLowerCase()));
                                                    return 1;
                                                })
                                        )
                                )
                                .then(literal("show")
                                        .executes(context -> {
                                            if(Main.SONG_PLAYER.instrumentMap.isEmpty()) {
                                                context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".no_mapped_instruments"));
                                                return 1;
                                            }
                                            ...
                                            context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".mapped_instruments", maps.toString()));
                                            return 1;
                                        })
                                )
                                .then(literal("clear")
                                        .executes(context -> {
                                            ...
                                            context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".instrument_maps_cleared"));
                                            return 1;
                                        })
                                )
                        )
                        ...
                        .then(literal("loop")
                                .executes(context -> {
                                    context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".loop_status", Main.SONG_PLAYER.loopSong ? "yes" : "no"));
                                    return 1;
                                })
                                .then(literal("yes")
                                    .executes(context -> {
                                        Main.SONG_PLAYER.loopSong = true;
                                        context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".loop_enabled"));
                                        return 1;
                                    }))
                                .then(literal("no")
                                    .executes(context -> {
                                        Main.SONG_PLAYER.loopSong = false;
                                        context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".loop_disabled"));
                                        return 1;
                                    }))
                        )
        );
    ...
    private static boolean isLoading(CommandContext<ServerCommandSource> context) {
        if (SongLoader.loadingSongs) {
            context.getSource().sendMessage(Text.translatable(Main.MOD_ID + ".still_loading"));
